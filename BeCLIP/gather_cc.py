...
### Implementation from https://github.com/mlfoundations/open_clip
def grab(line):
    """
    ...
    if os.path.exists(PATH+"/%s/%d/%d.jpg"%(split,uid%1000,uid)):
        print("Finished", uid)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PATH):
        ...
        for i in range(1000):
            os.mkdir(os.path.join(PATH,"train", str(i)))
            os.mkdir(os.path.join(PATH,"val", str(i)))
    ...
    for tsv in sys.argv[2:]:
        ...
        for row in results:
            ...
            id, caption, url = row
            fp = os.path.join(PATH, split, str(id % 1000), str(id) + ".jpg")
